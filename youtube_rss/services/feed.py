...
# RSS File
async def get_rss_file_path(feed_id: str) -> Path:
    """
...
async def get_rss_file(feed_id: str) -> Path:
    """
    Returns a validated rss file.
    """
    rss_file = await get_rss_file_path(feed_id=feed_id)
...
async def delete_rss_file(feed_id: str):
    rss_file = await get_rss_file_path(feed_id=feed_id)
    rss_file.unlink()


async def build_rss_file(feed_id: str, use_cache=USE_CACHE) -> Path:
    """
    Builds a .rss file for source_id, saves it to disk.
    """
    feed = YoutubeFeed(feed_id=feed_id)
    await feed.generate(use_cache=use_cache)
    return await feed.save()


async def build_all_rss_files() -> None:
    """
    Build .rss files for all sources, save to disk."""
    sources = source_crud.get_all()
    for source in sources:
        await build_rss_file(feed_id=source.source_id, use_cache=False)
...
class YoutubeFeed:
    ...
    async def generate_rss_header(
        ...
        feed.podcast.itunes_author(itunes_author=source_info_dict["uploader"])  # type: ignore
        thumbnail = source_info_dict["thumbnails"][2]["url"]
        feed.podcast.itunes_image(itunes_image=thumbnail)  # type: ignore
        return feed

    async def generate_rss_posts(
        ...
        for item in entries:
            if item.get("entries"):
                ...
                for video in item_entires:
                    feed = await self.generate_rss_post(feed=feed, video=video)
            else:
                # Handle as List of Videos
                feed = await self.generate_rss_post(feed=feed, video=item)
        return feed

    async def generate_rss_post(self, feed: FeedGenerator, video: dict[str, Any]) -> FeedGenerator:
        """
    ...
    async def generate(self, use_cache: bool) -> None:
        """
        Generate a FeedGenerator Feed
        """
        source = await get_source(source_id=self.feed_id)
        ...
        self.feed = await self.generate_rss_header(
            feed=self.feed, source=source, source_info_dict=source_info_dict
        )
        self.feed = await self.generate_rss_posts(feed=self.feed, source_info_dict=source_info_dict)

    async def save(self) -> Path:
        """
        Saves a generated feed to file. Returns file path
        """
        rss_file = await get_rss_file_path(feed_id=self.feed_id)
        if not self.feed:
            await self.generate(use_cache=True)
        await self.feed.rss_file(rss_file, encoding="UTF-8", pretty=True)
        return rss_file
