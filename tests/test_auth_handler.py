...
def test_get_password_hash(auth_handler):
...
def test_verify_password(auth_handler):
    plain_password = "password"
    hashed_password = auth_handler.get_password_hash(plain_password)
    assert auth_handler.verify_password(plain_password, hashed_password)


def test_encode_access_token(auth_handler):
    user_id = "user1"
...
def test_encode_refresh_token(auth_handler):
    user_id = "user1"
...
def test_decode_access_token(auth_handler):
    user_id = "user1"
    token = auth_handler.encode_access_token(user_id)
    assert auth_handler.decode_access_token(token) == user_id


def test_decode_refresh_token(auth_handler):
    user_id = "user1"
    token = auth_handler.encode_refresh_token(user_id)
